/*
 * SISTEMA DE SEGURIDAD VEHICULAR BMW E36 - ESP32 MAESTRO V4.7 FINGER LOGIC & ELEGANT LCD
 * * CAMBIOS V4.7:
 * - Sensor de Huella: Inicia APAGADO (Disabled). Solo se activa si el vehículo NO está en Stage 0 (Bloqueado).
 * - UI LCD 16x2: Nueva función de centrado automático de texto para una apariencia simétrica y elegante.
 * - Stage 3: Detección por voltaje (Mantenido de V4.6).
 */
...
// Huella
bool fingerEnabled = false; // INICIA APAGADA (V4.7)   
...
void setup() {
  ...
  if (Wire.endTransmission() == 0) {
    ...
    lcd.setCursor(2, 0); lcd.print("BMW SECURITY"); 
  } else {
  ...
  connectA9G_MQTT();
  delay(1000);
  changeStage(0); // Esto apagará la huella inicialmente
...
void toggleStandby(bool state) {
    ...
    if (state) {
        ...
        // Reactivar huella solo si no estamos bloqueados (se revisa abajo)
        if(vehicleStage > 0) fingerEnabled = true;
        fingerResetCount = 0;
    }
    sendStatusJSON();
}
...
void waitForA9G_Boot() {
    updateLCD("SISTEMA", "INICIANDO A9G...");
    logMsg("Esperando A9G...");
    ...
    while(!a9gReady && attempts < 30) {
        ...
        if(!a9gReady) {
            attempts++;
            oled.clearDisplay(); 
            oled.setCursor(0,0); oled.print("Booting A9G..."); 
            oled.setCursor(0,20); oled.print("Intento: " + String(attempts));
            oled.display();
            delay(500);
        }
    }
    ...
    if(a9gReady) {
        logMsg("A9G Detectado!");
        updateLCD("A9G", "DETECTADO OK");
        delay(1000);
    } else {
        logMsg("A9G Timeout!");
        updateLCD("A9G", "NO RESPONDE");
    }
}
...
void changeStage(int newStage) {
  ...
  // LOGICA HUELLA V4.7
  // Si estamos bloqueados (0) o en standby, apagar huella.
  if (vehicleStage == 0 || standbyMode) {
      fingerEnabled = false; 
  } else {
      // Si estamos desbloqueados (1, 2, 3) y el hardware está sano, encender.
...
// --- NUEVA FUNCION PARA ACTIVAR PIN CHAPA ---
void pulseChapaOutput() {
    ...
    // Mantiene encendido por 1 segundo (ajusta si necesitas más tiempo)
    delay(1000); 
    ...
    // Restaurar pantalla anterior si aplica
    if(vehicleStage == 0) updateLCD("SISTEMA", "BLOQUEADO");
    updateOLEDMenu();
}
...
void handleFingerprint() {
  // Si está deshabilitada (Bloqueado/Standby), no hacer nada
...
void toggleDisplays(bool state) {
  ...
  if (!displaysEnabled) {
    ...
    Wire.begin();
    lcd.init(); lcd.backlight();
    // Mensaje Reactivado Centrado
    lcd.setCursor(3,0); lcd.print("REACTIVADO");
...
// --- NUEVA FUNCION UPDATE LCD ELEGANTE ---
// Centra automáticamente el texto en pantalla de 16 caracteres
void updateLCD(String line1, String line2) { 
  if(!displaysEnabled || !displaysHealthy) return;
  lcd.clear(); 
  
  // Calcular padding para linea 1
  int pad1 = (16 - line1.length()) / 2;
  if(pad1 < 0) pad1 = 0;
  
  // Calcular padding para linea 2
...
void sendStatusJSON() {
  ...
  // JSON V4.6: Se agrega 'go' (GPS On Status) y 's' (Standby)
  // 'go': 1 si gpsEnabled es true, 0 si false.
  ...
  if (currentMenuState == MAIN_MENU) {
    for (int i = 0; i < MAIN_MENU_ITEMS; i++) {
      ...
      if (mainMenuIndex == 0) { // CONTROL
          // MODIFICADO: subItems[4] ahora es "Activar Chapa" en vez de "Puerta"
      ...
      // SCROLL LOGIC FIX
      int maxVisible = 4; // Maximas lineas visibles en pantalla
      int startIdx = 0;
      ...
      for (int i = 0; i < maxVisible; i++) {
          ...
          if (itemIdx >= count) break; // No dibujar mas alla del final
...
void handleMenuNavigation() {
  ...
  if (digitalRead(BTN_DOWN) == LOW) {
    if (currentMenuState == MAIN_MENU) { mainMenuIndex++; if (mainMenuIndex >= MAIN_MENU_ITEMS) mainMenuIndex = 0; }
        ...
        subMenuIndex++; 
        // Logica para limitar el indice maximo del submenu
        int maxItems = 0;
